...
def pHCurve(rawCountData, pHRange, outputDirName, saveType):
    ...
    try:
        fitData, covariance = curve_fit(hendersonHasselBalch, pHRange, edgeCutoffData, bounds=([0, -1], [3, 20]))
    except RuntimeError:
        print("Fit Failed")
        return
...
def checkForESV(searchDirectory):
    for i in os.listdir(searchDirectory):
        if i.__contains__("up.esv"):  # TODO: May have to change this depending on pdb file name (searching for just esv will cause an error because of the backup esvs)
